...
class PictureHistogram{
    ...
    private int getKey(FilterablePicture picture, int i, int j)   {
        Color color = picture.getAt(i,j);
        switch (picture.getFilterType())    {
            case RHistogram: return color.getRed();
    ...
    private IntervalXYDataset createDataset(FilterablePicture picture) {
        final XYSeries series = new XYSeries("");
        int[] data = new int[266];
        for (int i = 0; i < picture.width(); i++)
            for (int j = 0; j < picture.height(); j++)
                data[getKey(picture, i, j)]++;
